...
-- Suggested location for procedureMaster DB : 
-- Usage:           
-- From within ANY Database: 
--      exec sp_GetTopMissingIndexesWithQueryStore  N'YourDatabaseName'
-- From within your current Database: 
--      exec sp_GetTopMissingIndexesWithQueryStore
-- =============================================================================

Create OR Alter Procedure dbo.sp_GetTopMissingIndexesWithQueryStore
...
WITH BusinessHoursStats AS (
    ...
    FROM [{$DBName$}].sys.dm_db_missing_index_group_stats_query AS miq
    INNER JOIN [{$DBName$}].sys.query_store_query AS qs
        ON qs.query_hash = miq.query_hash
    INNER JOIN [{$DBName$}].sys.query_store_plan AS qp
        ON qp.query_id = qs.query_id
    INNER JOIN [{$DBName$}].sys.query_store_runtime_stats AS qrs
        ON qrs.plan_id = qp.plan_id
    INNER JOIN [{$DBName$}].sys.query_store_runtime_stats_interval AS rsi
...
SELECT TOP 25
    ...
    (
        ...
        FROM [{$DBName$}].sys.dm_db_index_usage_stats AS u
...
FROM [{$DBName$}].sys.dm_db_missing_index_groups AS mig
INNER JOIN [{$DBName$}].sys.dm_db_missing_index_group_stats AS migs
    ON migs.group_handle = mig.index_group_handle
INNER JOIN [{$DBName$}].sys.dm_db_missing_index_details AS mid
    ON mig.index_handle = mid.index_handle
INNER JOIN [{$DBName$}].sys.objects AS tab
    ON mid.object_id = tab.object_id
INNER JOIN [{$DBName$}].sys.schemas AS sche
